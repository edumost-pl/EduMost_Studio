#!/usr/bin/env python3
"""Validate seed_klasa5_biologia.sql without touching the database."""
...
SEED = ROOT / "electron/database/seed/seed_klasa5_biologia.sql"
EXPECTED_SECTIONS = 5
EXPECTED_TOPICS = 33
EXPECTED_RELATIONS = 64  # 32 PREREQUISITE + 32 NEXT
...
def validate(path: Path) -> dict:
    ...
    topic_codes = re.findall(r"SELECT s\.id, '(BIO5-[A-Z]{3}-\d{3})'", text)
    topic_display_orders = list(range(1, EXPECTED_TOPICS + 1)) if len(topic_codes) == EXPECTED_TOPICS else []

    curriculum_orders = [
        int(m.group(1))
        for m in re.finditer(
            r"INSERT OR IGNORE INTO Curriculum[\s\S]*?, (\d+), 1\s*FROM Topics t WHERE t\.code = '(BIO5-[A-Z]{3}-\d{3})'",
            text,
        )
    ]

    lesson_codes = re.findall(
        r"SELECT\s+'(BIO5-L\d{3})', sub\.id",
        text,
    )
...
def print_report(r: dict) -> bool:
    ok = True
    print("=" * 72)
    print("BIO5 SEED — RAPORT WALIDACYJNY (plik, bez bazy)")
    print("=" * 72)
    print(f"\nPlik: {r['path']}")
    ...
    if r["topic_order_gaps"]:
        ok = False
        print(f"\nLuki display_order (Topics): {r['topic_order_gaps']}")
