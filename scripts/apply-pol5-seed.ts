#!/usr/bin/env tsx
/**
 * Import seed_klasa5_jezyk_polski.sql into the active EduMost Studio database.
...
const polSeedPath = path.join(__dirname, '../electron/database/seed/seed_klasa5_jezyk_polski.sql');
...
function validate(db: Database.Database): {
  ...
  const topics = count(db, `SELECT COUNT(*) AS c FROM Topics WHERE code LIKE 'POL5-%'`);
  const curriculumUserQuery = count(
    db,
    `SELECT COUNT(*) AS c FROM Curriculum
     WHERE school_class = 5 AND curriculum_code LIKE 'POL5-%'`,
  );
  const curriculum = count(
    db,
    `SELECT COUNT(*) AS c FROM Curriculum c
     INNER JOIN Topics t ON t.id = c.topic_id
     WHERE c.school_class = 5 AND t.code LIKE 'POL5-%'`,
  );
  const lessons = count(db, `SELECT COUNT(*) AS c FROM Lessons WHERE code LIKE 'POL5-L%'`);
  const lessonTopics = count(
    db,
    `SELECT COUNT(*) AS c FROM LessonTopics
     WHERE topic_id IN (SELECT id FROM Topics WHERE code LIKE 'POL5-%')`,
  );

  if (topics !== EXPECTED.topics) errors.push(`Topics POL5: ${topics} (expected ${EXPECTED.topics})`);
  if (curriculum !== EXPECTED.curriculum) {
    errors.push(`Curriculum POL5 (via Topics): ${curriculum} (expected ${EXPECTED.curriculum})`);
  }
  if (lessons !== EXPECTED.lessons) errors.push(`Lessons POL5: ${lessons} (expected ${EXPECTED.lessons})`);
  if (lessonTopics !== EXPECTED.lessonTopics) {
    errors.push(`LessonTopics POL5: ${lessonTopics} (expected ${EXPECTED.lessonTopics})`);
  }
  ...
  const firstTopic = db
    .prepare(
      `SELECT code, name_pl FROM Topics WHERE code LIKE 'POL5-%'
       ORDER BY CAST(SUBSTR(code, -3) AS INTEGER) ASC LIMIT 1`,
    )
...
function main(): void {
  ...
  if (!fs.existsSync(polSeedPath)) {
    console.error(`Seed file not found: ${polSeedPath}`);
    process.exit(1);
  }
  ...
  try {
    ...
    const existingPol = count(db, `SELECT COUNT(*) AS c FROM Topics WHERE code LIKE 'POL5-%'`);
    if (existingPol > 0) {
      console.error(`POL5 already present (${existingPol} topics). Aborting to avoid duplicates.`);
      process.exit(1);
    }

    console.log('=== POL5 IMPORT ===\n');
    console.log(`Target DB: ${dbPath}\n`);

    backupPath = createBackup(dbPath);
    console.log(`Backup created: ${backupPath}\n`);

    console.log('Loading seed_klasa5_jezyk_polski.sql …');
    ...
    console.log(`Topics POL5:         ${result.topics}`);
    console.log(
      `Curriculum POL5:     ${result.curriculumUserQuery}  (zapytanie: curriculum_code LIKE 'POL5-%')`,
    );
    console.log(
      `Curriculum POL5:     ${result.curriculum}  (via Topics JOIN, school_class = 5)`,
    );
    console.log(`Lessons POL5:        ${result.lessons}`);
    console.log(`LessonTopics POL5:   ${result.lessonTopics}`);
    ...
    if (result.firstTopic) {
      console.log(`Pierwszy POL5:       ${result.firstTopic.code} — ${result.firstTopic.name_pl}`);
    }
    if (result.lastTopic) {
      console.log(`Ostatni POL5:        ${result.lastTopic.code} — ${result.lastTopic.name_pl}`);
    }
