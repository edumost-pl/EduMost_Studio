#!/usr/bin/env python3
"""Validate seed_klasa5_jezyk_polski.sql without touching the database."""
...
SEED = ROOT / "electron/database/seed/seed_klasa5_jezyk_polski.sql"
...
def validate(path: Path) -> dict:
    ...
    section_codes = re.findall(
        r"INSERT OR IGNORE INTO Sections[\s\S]*?code = '([A-Z]{3})'",
        text,
    )
    # Sections use SELECT ... code in INSERT - better pattern:
    section_inserts = len(re.findall(r"INSERT OR IGNORE INTO Sections", text))

    topic_codes = re.findall(r"SELECT s\.id, '(POL5-[A-Z]{3}-\d{3})'", text)
    topic_display_orders = [int(c.rsplit("-", 1)[-1]) for c in topic_codes]

    curriculum_orders = [
        int(m.group(1))
        for m in re.finditer(
            r"INSERT OR IGNORE INTO Curriculum[\s\S]*?, (\d+), 1\s*FROM Topics t WHERE t\.code = '(POL5-[A-Z]{3}-\d{3})'",
            text,
        )
    ]

    lesson_codes = re.findall(r"VALUES \(\s*'(POL5-L\d{3})'", text)
    ...
    return {
        ...
        "section_insert_count": section_inserts,
    }
...
def print_report(r: dict) -> bool:
    ok = True
    print("=" * 72)
    print("POL5 SEED — RAPORT WALIDACYJNY (plik, bez bazy)")
    print("=" * 72)
    print(f"\nPlik: {r['path']}")
